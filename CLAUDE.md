...
**What's left**: real Paystack test keys to exercise a live charge and a live subaccount creation; background-job handling for bulk upload; scheduled reports (depends on the Background Jobs decision); a platform-wide config store (`PlatformSettings` currently holds only the marketplace fee — the broader platform-name/support-email/feature-toggle version is still unbuilt); a branching/PR workflow given the v0 auto-push-to-main behavior.
