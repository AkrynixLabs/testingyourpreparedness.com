  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold tracking-tight">{resultData.title}</h1>
          <p className="text-muted-foreground">
            Completed on {resultData.date} - {resultData.duration} taken
          </p>
        </div>
      </div>
          ...
          <CardContent>
            <div className="flex flex-col items-center gap-6 md:flex-row md:justify-around">
              ...
              <div className="grid gap-4">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xl font-bold">{resultData.duration}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="text-center">
                ...
                <div className="mt-4">
                  <p className="text-3xl font-bold text-primary">#{resultData.rank}</p>
                  <p className="text-sm text-muted-foreground">
                    out of {resultData.totalStudents} students
                  </p>
                </div>
                <p className="mt-2 text-sm">
                  Top <span className="font-semibold text-emerald-600">{resultData.percentile}%</span> percentile
                </p>
              </div>
            </div>
          </CardContent>
          ...
          <CardContent className="space-y-4">
            ...
            <div>
              <div className="flex justify-between text-sm mb-1">
                ...
                <span className="font-medium">{resultData.classAverage}%</span>
              </div>
              <Progress value={resultData.classAverage} className="h-3 [&>div]:bg-muted-foreground" />
            </div>
            <Separator />
            <div className="space-y-2 text-sm">
              <div className="flex justify-between">
                <span className="text-muted-foreground">Highest Score</span>
                <span className="font-medium text-emerald-600">{resultData.highestScore}%</span>
              </div>
              <div className="flex justify-between">
                <span className="text-muted-foreground">Lowest Score</span>
                <span className="font-medium text-red-600">{resultData.lowestScore}%</span>
              </div>
              <div className="flex justify-between">
                <span className="text-muted-foreground">Your Position</span>
                <span className="font-medium">
                  {percentage > resultData.classAverage ? (
        ...
        <CardContent>
          <div className="h-[300px]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={resultData.topicBreakdown} layout="vertical">
          ...
          <div className="mt-6 grid gap-3 md:grid-cols-2 lg:grid-cols-3">
            {resultData.topicBreakdown.map((topic) => (
              <div
                key={topic.topic}
